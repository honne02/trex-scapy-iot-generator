...
install_trex() {
    echo "=== Установка зависимостей и TRex ==="
...
release_dpdk_interfaces() {
    ...
    echo "=== Отключение интерфейсов от DPDK ==="
    echo "Утилита: $bind_tool"
    echo "Текущий статус:"
    ...
    if [ -z "$pci_list" ]; then
        echo "В $TREX_CFG не найдены PCI-адреса."
        read -r -p "Введи PCI-адреса интерфейсов через пробел (например 0000:03:00.0 0000:04:00.0): " pci_list
    else
        echo "PCI-адреса из $TREX_CFG:"
        echo "$pci_list"
    fi
    ...
    echo ""
    echo "Перед продолжением останови TRex server, если он еще запущен."
    read -r -p "Продолжить возврат интерфейсов в Linux-драйвер? [y/N]: " confirm
    ...
    for pci_addr in $pci_list; do
        ...
        if [ -n "$auto_driver" ]; then
            read -r -p "Linux-драйвер для $pci_addr [Enter = $auto_driver]: " target_driver
            target_driver=${target_driver:-$auto_driver}
        else
            echo "Не удалось автоматически определить Linux-драйвер для $pci_addr."
            read -r -p "Введи драйвер вручную (например ixgbe, i40e, e1000e, virtio-pci) или Enter чтобы пропустить: " target_driver
        fi
    ...
    python3 "$bind_tool" -s || true
    echo "Готово. Если Linux-интерфейсы появились, подними их через ip link set <iface> up при необходимости."
}
...
load_existing_trex_cfg() {
    if [ ! -f "$TREX_CFG" ]; then
        echo "Ошибка: $TREX_CFG не найден."
        echo "Сначала запусти режим настройки, чтобы создать YAML."
        exit 1
    fi
    ...
    if [ -z "$MY_MAC1" ]; then
        read -r -p "Не удалось прочитать src_mac из YAML. Введи MAC для генерации PCAP: " MY_MAC1
    else
...
print_launch_instructions() {
    ...
    if [ "$MODE" == "2" ]; then
        echo ""
        echo "ПОСЛЕ ОСТАНОВКИ COMBAT MODE:"
        echo "   sudo bash $LAUNCH_DIR/setup_generator_trex_IoT.sh"
        echo "   # выбери пункт 4, чтобы вернуть DPDK-интерфейсы в Linux-драйвер"
    fi
    echo "======================================================="
}
...
echo "4) Отключить интерфейсы от DPDK и вернуть в Linux-драйвер"
...
case "$START_MODE" in
    ...
    4)
        ...
        ;;
    *)
        echo "Неизвестный пункт: $START_MODE"
        exit 1
        ;;
esac
